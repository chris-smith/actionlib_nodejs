...
function getMessage(messageType) {
  if (!isConfigured()) {
    throw new Error('Unable to query ActionConfig before it is configured!');
  }
  // else
  return ACTION_CONF.messages.getMessage(messageType);
}

function getMessagesForAction(actionType) {
  return {
    goal: getMessage(actionType + 'Goal'),
    result: getMessage(actionType + 'Result'),
    feedback: getMessage(actionType + 'Feedback'),
    actionGoal: getMessage(actionType + 'ActionGoal'),
    actionResult: getMessage(actionType + 'ActionResult'),
    actionFeedback: getMessage(actionType + 'ActionFeedback')
  };
}
...
exportItem.get = get;
exportItem.getMessagesForAction = getMessagesForAction;
